...
export function VaultAssetClient({ vaultAssetId }: VaultAssetClientProps) {
  ...
  if (vaultAssetQuery.isLoading) {
    return <div className="rounded-[2rem] bg-[#111] p-8 text-[#f0ede6]">Loading vault asset...</div>;
  }
      ...
      <div className="grid gap-8 xl:grid-cols-[minmax(0,1fr)_360px]">
          ...
          <div className="rounded-[2rem] border border-[rgba(255,255,255,0.07)] bg-[#111] p-6 backdrop-blur">
            <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <h2 className="mt-3 font-[family-name:var(--font-bebas-neue)] tracking-[0.06em] text-2xl font-semibold text-[#f0ede6]">
                  Preview vault content
                </h2>
              </div>
              <button
                className="inline-flex items-center gap-2 rounded-full bg-[#c8392b] px-4 py-2 text-sm font-semibold text-[#f0ede6]"
            ...
            <div className="mt-5 rounded-3xl border border-[rgba(255,255,255,0.07)] bg-[rgba(255,255,255,0.03)] p-5">
              <p className="text-sm font-semibold text-[#f0ede6]">
                Vault the content, not the chain record.
              </p>
              <p className="mt-2 text-sm leading-6 text-[rgba(240,237,230,0.55)]">
            ...
            <div className="mt-6">
              {previewFile && previewSrc ? (
                ...
                <div className="flex h-72 items-center justify-center rounded-3xl border border-dashed border-[rgba(200,57,43,0.3)] bg-[rgba(255,255,255,0.03)] p-8 text-center text-sm text-[rgba(240,237,230,0.55)]">
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2">
              {vaultAsset.vaultFiles.map((entry) => (
                <div key={entry.id} className="rounded-3xl border border-[rgba(255,255,255,0.07)] bg-[#161616] p-4">
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      <p className="text-sm font-semibold text-[#f0ede6]">
                        {entry.file.filename}
                      </p>
                      <p className="mt-1 text-xs uppercase tracking-[0.22em] text-[rgba(240,237,230,0.35)]">
                        {prettyRole(entry.role)}
                      </p>
                    </div>
                    <span className="rounded-full bg-[rgba(255,255,255,0.05)] px-3 py-1 text-xs font-semibold text-[rgba(240,237,230,0.55)]">
                      {entry.file.isEncrypted ? "Encrypted" : "Plain"}
                    </span>
                  </div>
                  <div className="mt-4 flex flex-wrap gap-2 text-xs text-[rgba(240,237,230,0.55)]">
                    <span className="rounded-full bg-[rgba(255,255,255,0.05)] px-3 py-2">
                  ...
                  <div className="mt-4 flex gap-3">
                    {entry.role === VAULT_FILE_ROLES.TEASER ? (
                      ...
                      <>
                        ...
                        <button
                          className="inline-flex items-center gap-2 rounded-full bg-[rgba(255,255,255,0.05)] px-4 py-2 text-sm font-semibold text-[rgba(240,237,230,0.55)]"
                          disabled={isLoadingPreview}
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <div className="rounded-[2rem] border border-[rgba(255,255,255,0.07)] bg-[#111] p-6 backdrop-blur">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <h2 className="mt-1 font-[family-name:var(--font-bebas-neue)] tracking-[0.06em] text-2xl font-semibold text-[#f0ede6]">
              ...
              <div className="mt-4 grid gap-4">
                <select
                  className="rounded-2xl border border-[rgba(255,255,255,0.07)] bg-[#161616] px-4 py-3 text-sm text-[#f0ede6] outline-none"
                ...
                <textarea
                  className="h-28 rounded-3xl border border-[rgba(255,255,255,0.07)] bg-[#161616] px-4 py-3 text-sm text-[#f0ede6] outline-none"
                  onChange={(event) => setDescription(event.target.value)}
                  placeholder="Describe what collectors unlock here."
                  value={description}
                />
                <label className="flex items-center gap-3 rounded-2xl bg-[rgba(255,255,255,0.03)] px-4 py-3 text-sm text-[rgba(240,237,230,0.55)]">
                  <input
                    checked={encrypt}
                    className="h-4 w-4"
                ...
                <button
                  className="rounded-2xl bg-[#c8392b] px-4 py-3 text-sm font-semibold text-[#f0ede6]"
            ...
            <div className="rounded-[2rem] border border-[rgba(255,255,255,0.07)] bg-[#111] p-6 backdrop-blur">
              ...
              {shareType === SHARE_TYPES.PASSWORD ? (
                <input
                  className="mt-4 w-full rounded-2xl border border-[rgba(255,255,255,0.07)] bg-[#161616] px-4 py-3 text-sm text-[#f0ede6] outline-none"
              ...
              <div className="mt-5 space-y-3">
                {vaultAsset.shares.map((share) => (
                  <div
                    key={share.id}
                    className="flex items-center justify-between rounded-3xl border border-[rgba(255,255,255,0.07)] bg-[rgba(255,255,255,0.03)] px-4 py-3"
                  >
                    <div className="flex items-center gap-2 text-sm text-[rgba(240,237,230,0.55)]">
                      ...
                      <Link className="font-semibold text-[#f0ede6]" href={`/share/${share.token}`}>
                        {share.shareType.toLowerCase()} link
                      </Link>
                    </div>
                    <button
                      className="rounded-full bg-[#161616] p-2 text-[rgba(240,237,230,0.55)]"
                      onClick={() =>
                ...
                {vaultAsset.shares.length === 0 ? (
                  <div className="rounded-3xl bg-[rgba(255,255,255,0.03)] p-4 text-sm text-[rgba(240,237,230,0.55)]">
        ...
        <aside className="space-y-6">
          <div className="rounded-[2rem] border border-[rgba(255,255,255,0.07)] bg-[#111] p-6 backdrop-blur">
            ...
            <div className="mt-4 rounded-3xl bg-[rgba(255,255,255,0.03)] p-5">
              <p className="text-sm font-semibold text-[#f0ede6]">
            ...
            <div className="mt-5 grid gap-3">
              <div className="rounded-3xl bg-[rgba(255,255,255,0.03)] p-4">
                <p className="text-sm text-[rgba(240,237,230,0.35)]">Public preview mode</p>
              ...
              <div className="rounded-3xl bg-[rgba(255,255,255,0.03)] p-4">
                <p className="text-sm text-[rgba(240,237,230,0.35)]">Protected files</p>
                <p className="mt-2 text-lg font-semibold text-[#f0ede6]">
                  {vaultAsset.vaultFiles.length}
                </p>
              </div>
              <div className="rounded-3xl bg-[rgba(255,255,255,0.03)] p-4">
                <p className="text-sm text-[rgba(240,237,230,0.35)]">Has teaser content</p>
                <p className="mt-2 text-lg font-semibold text-[#f0ede6]">
                  {teaserFile ? "Yes" : "Not yet"}
                </p>
              </div>
              <div className="rounded-3xl bg-[rgba(255,255,255,0.03)] p-4">
                <p className="text-sm text-[rgba(240,237,230,0.35)]">Has protected preview</p>
                <p className="mt-2 text-lg font-semibold text-[#f0ede6]">
          ...
          <div className="rounded-[2rem] border border-[rgba(255,255,255,0.07)] bg-[#111] p-6 backdrop-blur">
            ...
            <div className="mt-4 space-y-3">
              {vaultAsset.accessLogs.map((log) => (
                <div key={log.id} className="rounded-3xl bg-[rgba(255,255,255,0.03)] p-4">
                  <p className="text-sm font-semibold text-[#f0ede6]">
                    {log.accessType.replaceAll("_", " ").toLowerCase()}
                  </p>
                  <p className="mt-1 text-sm text-[rgba(240,237,230,0.35)]">{formatDate(log.createdAt)}</p>
                  <p className="mt-1 text-xs text-[rgba(240,237,230,0.25)]">
